...
class IrisDecoderTest {
    ...
    @Test
    void testPacket84WithProductionFrameA() {
        String packet = ">84=1558885+2192325-0794445600440000581320070116045753030000000000<";
        Mono<TelemetriaVehiculo> result = irisDecoder.decode(packet.getBytes(StandardCharsets.US_ASCII), channel);

        StepVerifier.create(result)
                .assertNext(telemetria -> {
                    assertEquals("1558885", telemetria.getVehicleId());
                    assertEquals(21.92325, telemetria.getLatitude(), 0.000001);
                    assertEquals(-79.44456, telemetria.getLongitude(), 0.000001);
                    assertNotNull(telemetria.getTimestamp());
                })
                .verifyComplete();
    }
    ...
    @Test
    void testPacket84WithProductionFrameB() {
        String packet = ">84=1558885+2192326-0794445800440030441320070116045747030000000000<";
        Mono<TelemetriaVehiculo> result = irisDecoder.decode(packet.getBytes(StandardCharsets.US_ASCII), channel);

        StepVerifier.create(result)
                .assertNext(telemetria -> {
                    assertEquals("1558885", telemetria.getVehicleId());
                    assertEquals(21.92326, telemetria.getLatitude(), 0.000001);
                    assertEquals(-79.44458, telemetria.getLongitude(), 0.000001);
                    assertNotNull(telemetria.getTimestamp());
                })
                .verifyComplete();
    }
